...
# # define P during runtime
# GOAL_WEIGHTS = torch.tensor((20.5, 20.5, 5.5, 20.0), dtype=torch.float32)  # nx
# CTRL_PENALTY = torch.tensor((0.01, 100), dtype=torch.float32) # nu
# q = torch.cat((GOAL_WEIGHTS, CTRL_PENALTY))  # nx + nu
# Q = torch.diag(q).repeat(T, N_BATCH, 1, 1)  # T x B x nx+nu x nx+nu
...
class Controller:
    ...
    def calc_ref_trajectory(self, state, cx, cy, cyaw, sp, dl, pind):
        ...
        ind, _ = Controller.calc_nearest_index(self, state, cx, cy, cyaw, pind, N_IND_SEARCH)
    ...
    def mpc_control(self, ref_traj, x0):
        ...
        ctrl =  mpc.MPC(NX, NU, T, u_lower=U_LOWER, u_upper=U_UPPER, lqr_iter=LQR_ITER,
                ...
                n_batch=N_BATCH, backprop=False, verbose=VERBOSE_MPC, u_init=self.u_init,
    ...
    def MPC_Controller (self, vehicle_state, path):
        ...
        if self.mpc_initialize == 0:
            self.target_ind, _ = self.calc_nearest_index(vehicle_state, cx, cy, cyaw, 0, len(cx))
            # self.target_ind = 0
...
if __name__ == '__main__':
    ...
    while not done:
        ...
        obs, step_reward, done, info = env.step(np.array([[steer, SPEED_FACTOR * speed]]))
        laptime += step_reward
