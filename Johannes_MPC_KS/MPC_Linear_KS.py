...
MAX_SPEED = 1.8                    # maximum speed [m/s]
...
class Controller:
    ...
    def calc_ref_trajectory(self, state, cx, cy, cyaw, sp, dl, pind):
        ...
        for i in range(T + 1):
            ...
            if (ind + dind) < ncourse:
                ...
                # dref[0, i] = math.atan2(WB*ref_traj[3, i], ref_traj[2, i])
    ...
    def get_kinematic_model_matrix(v, phi, delta):
        ...
        ************ Single Track Model: Linear - Kinematic ********
        ...
        #     if abs(eigenvalue) > 1:
        #         print("Error: A has instable eigenvalues: ", eigenvalues)
    ...
    def mpc_optimization(ref_traj, path_predict, x0, dref):
        ...
        if prob.status == cvxpy.OPTIMAL:
            mpc_x = Controller.get_nparray_from_matrix(x.value[0, :])
            mpc_y = Controller.get_nparray_from_matrix(x.value[1, :])
            mpc_v = Controller.get_nparray_from_matrix(x.value[2, :])
            mpc_yaw = Controller.get_nparray_from_matrix(x.value[3, :])
            mpc_a = Controller.get_nparray_from_matrix(u.value[0, :])
            mpc_delta = Controller.get_nparray_from_matrix(u.value[1, :])
        
        elif prob.status == cvxpy.OPTIMAL_INACCURATE:
            print("Not accurate solution")
            mpc_x = Controller.get_nparray_from_matrix(x.value[0, :])
            mpc_y = Controller.get_nparray_from_matrix(x.value[1, :])
            mpc_v = Controller.get_nparray_from_matrix(x.value[2, :])
            mpc_yaw = Controller.get_nparray_from_matrix(x.value[3, :])
    ...
    def MPC_Controller (self, vehicle_state, path):
        global debug_ox, debug_oy, debug_oyaw, debug_ov
        ...
        # print("Ref Path (x y v yaw):", ref_path[0][0:3], ref_path[1][0:3], ref_path[2][0:3], ref_path[3][0:3])
        ...
        debug_ox, debug_oy, debug_oyaw, debug_ov = ox, oy, oyaw, ov

        print("Ref path deviation: ", sum(abs(ref_path[0] - vehicle_state.x)),sum( abs(ref_path[1] - vehicle_state.y)))
        # print("ox:", ox[0], "oy:", oy[0])
        ...
        if debugplot == 1:
            plt.cla()
            # plt.axis([-40, 2, -10, 10])
            plt.axis([vehicle_state.x - 10, vehicle_state.x + 10, vehicle_state.y - 10, vehicle_state.y  + 10])
...
class LatticePlanner:
    ...
    def __init__(self, conf, env, controller, wb):
        self.conf = conf                        # Current configuration for the gym based on the maps
        self.controller = controller            # Controller object for the MPC Controller
...
if __name__ == '__main__':
    ...
    # -------------------------- SIMULATION LOOP  ----------------------------------------
    while not done:
        # Call the function for planning a path, only every Tth timestep
        if control_count >= (T+1):
        ...
        # print("poses_x:", obs['poses_x'], "poses_y:", obs['poses_y'], "poses_theta:", obs['poses_theta'])
        if(obs['collisions'] == 1):
        ...
        if conf_dict['logging'] == 'True':
            logging.logging(obs['poses_x'][0], obs['poses_y'][0], obs['poses_theta'][0], obs['linear_vels_x'][0],
                            ...
                            env.sim.agents[0].state[6], debug_ox[control_count], debug_oy[control_count], debug_oyaw[control_count], debug_ov[control_count])
